...
import { isRecoverySuppressedByRecentUserTransition } from "./user-transition-guard.js";
...
export function recoveryService(db: Db, deps: { enqueueWakeup: RecoveryWakeup }) {
  ...
  async function reconcileStrandedAssignedIssues() {
    ...
    for (const issue of candidates) {
      ...
      if (await isRecoverySuppressedByRecentUserTransition(db, issue.companyId, issue.id)) {
        result.skipped += 1;
        continue;
      }
